...
module.exports.omiseResources = function (config) {
	return {
		sources: require('./resources/Source')(config),
		tokens: require('./resources/Token')(config),
		customers: require('./resources/Customer')(config),
		charges: require('./resources/Charge')(config),
		schedules: require('./resources/Schedule')(config),
		disputes: require('./resources/Dispute')(config),
		transfers: require('./resources/Transfer')(config),
		transactions: require('./resources/Transaction')(config),
		recipients: require('./resources/Recipient')(config),
		events: require('./resources/Event')(config),
		links: require('./resources/Link')(config),
		account: require('./resources/Account')(config),
		balance: require('./resources/Balance')(config),
		search: require('./resources/Search')(config),
		capability: require('./resources/Capability')(config),
	};
};
	...
	return _.pick(
		{
			...
			schedules: function (resourceId, data, callback) {
				...
				if (resourceId) {
					...
					return api.get(
						_makeOptions(options, path('schedules'), data),
						callback
					);
				}
			},
			...
			capture: function (resourceId, callback) {
				return api.post(
					_nestedPathOptions(options, path(resourceId), 'capture'),
					callback
				);
			},
